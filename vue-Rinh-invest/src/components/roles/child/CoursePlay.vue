<template>
  <div class="min-h-screen flex flex-col md:flex-row">
    ...
    <div :class="['flex-grow p-6 shadow-md', backgroundClass, 'bg-transition']">
      ...
      <!-- Чат с прокруткой -->
      <div ref="chatContainer" class="space-y-4 overflow-y-auto h-96 pb-4">
    ...
    <div class="w-full md:w-1/3 bg-gray-100 p-6 border-l shadow-md flex flex-col">
      <h3 class="text-xl font-bold mb-4 text-center">Интерактивная консоль</h3>

      <!-- Консоль с прокруткой -->
      <div
        class="flex-grow bg-gray-800 text-white p-4 font-mono console-output overflow-y-auto h-64 pb-4"
      >
        <div
      ...
      <p class="text-sm text-center mb-2">Введите код в поле ниже:</p>

      <div ref="codeMirrorContainer" class="mt-2 mb-4"></div>
...
export default {
  data() {
    return {
      ...
      consoleCommands: [],
      currentEventIndex: 0,
      dynamicsOn: false, // Добавлено для управления динамикой
      events: [
        {
      ...
      failureSound: new Audio('path/to/failure-sound.mp3'),
    }
  },
  computed: {
    currentEvent() {
  ...
  methods: {
    ...
    toggleDynamics() {
      this.dynamicsOn = !this.dynamicsOn
      // Здесь можно добавить логику для включения/выключения динамики
    },
  },
  mounted() {
...
.bg-dark-room {
  background-image: url('@/assets/dark-room.jpg');
...
.bg-castle {
  background-image: url('@/assets/castle-image.jpg');
  background-size: cover;
  background-position: center;
}
